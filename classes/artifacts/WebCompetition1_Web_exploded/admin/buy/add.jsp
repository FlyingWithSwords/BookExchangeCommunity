    ...
    <input type="hidden" name="method" value="<%=method%>" /><input type="hidden" name="id" value="<%=id%>" />
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <%if(method.equals("upbuy")){ %> 
              ...
              <%} %>
              <%List flist=cb.getCom("select * from user where owner='用户' order by id desc",5);if(!flist.isEmpty()){for(int i=0;i<flist.size();i++){List list2=(List)flist.get(i);%>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <% if(method.equals("upbuy")){ %> 
              <option value="<%=bnid %>"><%=title %></option>
              <%}else{%> 
              <option value="">请选择</option>
              <%} %>
              ...
              <option value="<%=list0.get(0).toString() %>"><%=list0.get(6).toString() %></option>
              <%}} %>
        </td>
        </tr>
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="left" valign="middle" class="borderright borderbottom main-for">
        <input type="number" min="1" class="text-wordwide" name="price" value="<%=price %>" required/>
        </td>
      </tr>
