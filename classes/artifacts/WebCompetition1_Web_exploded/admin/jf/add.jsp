  ...
  <tr>
    ...
    <input type="hidden" name="method" value="<%=method%>" /><input type="hidden" name="id" value="<%=id%>" />
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <%if(method.equals("addjf")){ %>
              <%List flist0=cb.getCom("select * from jifen where id='"+id+"'",4);if(!flist0.isEmpty()){for(int i=0;i<flist0.size();i++){List list20=(List)flist0.get(i);%>
    		  <option value=<%=list20.get(1).toString() %>><%=list20.get(1).toString() %></option>
              <%}}}else{%> 
              <option value="">请选择</option>
              <%} %>
              <%List flist=cb.getCom("select * from user where owner='用户' order by id desc",4);if(!flist.isEmpty()){for(int i=0;i<flist.size();i++){List list2=(List)flist.get(i);%>
      ...
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="left" valign="middle" class="borderright borderbottom main-for"> 
        <input type="number" class="text-word" name="add" value="" required/>
        </td>
      </tr> 
