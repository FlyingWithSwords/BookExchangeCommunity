...
.helpBox .helpRight .helpPar .buybtn{
	border-radius:10px;border:0;background:#393D49;color:#fff;/* width:152px;height:41px; */padding:12px 36px;font-size:14px; cursor: pointer; transition: all 0.6s; -ms-transition: all 0.8s;
}
.helpBox .helpRight .helpPar .buybtn:hover{background:#191D29;}
</style>
   ...
   <div class="helpPar">
     ...
     <ol> 
       <li> 
		...
		发布人：<a href="<%=basePath%>jl.jsp?bkxwid=<%=pagelist20.get(0).toString() %>" target="menuFrame"><%=list2.get(2).toString() %></a><%}} %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		支付方式：<%=list2.get(10).toString() %><br><br><img src="<%=basePath+list2.get(11).toString() %>"  style="margin-bottom:50px;max-width:250px;myimg:expression_r(onload=function(){this.style.width=(this.offsetWidth > 250)?"250px":"auto"}"/></div>
		<div style="width:950px;word-wrap:break-word;"><p><%=list2.get(1).toString() %></p></div>
       </li>
       <li><div style="text-align:right;"><font style="color:red;font-size:24px;">￥<%=list2.get(8).toString() %></font><br/><br/><font style="text-align:right;color:#909090;"><%=list2.get(9).toString() %></font></div></li>
    	<% String mine=(String)session.getAttribute("user");
    		if(session.getAttribute("user")!=null){
    			if(!mine.equals(list2.get(2).toString())){
    	%>
    	<center><a href="<%=basePath%>ComServlet?inf=1&method=addbuy&username=<%=mine %>&bnid=<%=id %>&price=<%=list2.get(8).toString() %>" class="buybtn">购买</a></center>
    	<%}
    			else{%>
