...
<div style="margin-top:50px;"></div>
	...
	<% }
	else if(mine==null){%><div align="center" style="margin-top:50px"><h3 style="color:#7f7f7f;">请先登录！</h3></div><%}
	else {
	...
	<div class="layim-chat-main">
		...
		<%if(list3.get(1).toString().equals(caller)) {%>
			<li data-cid="<%=bi %>"><div class="layim-chat-user">
					<img src="<%=basePath+list2.get(9).toString()%>"><cite><%=caller %><i><%=list3.get(3).toString().substring(0, list3.get(3).toString().length()-2) %></i></cite>
				</div>
				<div class="layim-chat-text"><%=list3.get(2).toString() %></div></li>
		<%} 
		else if(list3.get(1).toString().equals(mine)){
		%>
			<li class="layim-chat-mine"><div class="layim-chat-user">
					<img src="<%=basePath+list2.get(9).toString()%>"><cite><i><%=list3.get(3).toString().substring(0, list3.get(3).toString().length()-2) %></i><%=mine %></cite>
				</div>
				<div class="layim-chat-text"><%=list3.get(2).toString() %></div></li>
				<%} %>
		</ul>
